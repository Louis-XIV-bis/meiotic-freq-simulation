...
combined_data = combined_data %>%
  filter(alpha == 1, s == 0.05) %>%
  mutate(mean_normalized = as.numeric(mean_normalized))
combined_data
...
theo_vs_simu <- ggplot(combined_data, aes(x = window, y = mean_normalized, color = origin, shape = origin, group = origin)) +
  ...
  geom_line(data = combined_data %>% filter(!window %in% c(1.1, 1.2)), linewidth = 1) +  # Line plot for most data
  geom_point(data = combined_data %>% filter(window %in% c(1.1, 1.2)), size = 4) +  # Points for special windows
  facet_grid(~h, scales = "free_x", labeller = labeller(h = function(x) paste0("h = ", x))) +  # Custom facet labels
  scale_x_continuous(
...
theo_simu_chr2 = ggplot(data_plot_alpha, aes(x = alpha, y = mean_normalized, color = origin)) +
  ...
  facet_grid(~h, labeller = labeller(h = function(x) paste0("h = ", x))) +  # Facet per s
