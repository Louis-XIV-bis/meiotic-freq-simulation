...
data_EGR = pi_table_neutral %>% filter(submodel == 1)
hist(as.vector(data_EGR$mean), probability = TRUE)
shapiro.test(data_EGR$mean) 
# p-value = 0.1575 => Gaussian 

data_100GR = pi_table_neutral %>% filter(submodel == 0.01)
hist(as.vector(data_100GR$mean), probability = TRUE)
shapiro.test(data_100GR$mean) 
# p-value = 0.5627 => Gaussian 

# Both gaussian : t.test
t.test(data_EGR$mean,data_100GR$mean)
# p-value = 6.016e-05 ==> significantly different 
# mean EGR = 3.967e-05, mean 100GR = 4.124e-05 =>  <4% !=
...
p = ggplot(pi_table_neutral, aes(x = submodel, y = mean, fill = submodel)) +
  geom_boxplot() +
