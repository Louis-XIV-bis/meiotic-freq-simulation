...
data_EGR_h0p4 = read_csv("rawSFS_local_EGR_h0p4.csv",col_names = F)
data_EGR_h0p4 = data_EGR_h0p4[,(1+41*(start_windows-1)):(1+41*(start_windows-1)+41*gap_windows-1)]

# Conversion to absolute values to frequency
for (i in c(1:100)){
  for (windows in c(0:(gap_windows-1))){
    list_col_windows = c((41*windows+1):(41*windows+41))
    data_EGR_h0p4[i,list_col_windows] = data_EGR_h0p4[i,list_col_windows]/sum(data_EGR_h0p4[i,list_col_windows])
  }
}
...
rawSFS_local = apply(data_EGR_h0p4, 2, mean) %>%
  as_tibble() %>% 
  slice(to_delet_freq) 
...
data_100GR_h0p4 = read_csv("rawSFS_local_100GR_h0p4.csv",col_names = F)
data_100GR_h0p4 = data_100GR_h0p4[,(1+41*(start_windows-1)):(1+41*(start_windows-1)+41*gap_windows-1)]

# Conversion to absolute values to frequency
for (i in c(1:100)){
  for (windows in c(0:(gap_windows-1))){
    list_col_windows = c((41*windows+1):(41*windows+41))
    data_100GR_h0p4[i,list_col_windows] = data_100GR_h0p4[i,list_col_windows]/sum(data_100GR_h0p4[i,list_col_windows])
  }
}
...
rawSFS_local = apply(data_100GR_h0p4, 2, mean) %>%
  as_tibble() %>% 
  slice(to_delet_freq) 
