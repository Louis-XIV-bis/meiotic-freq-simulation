...
GR=50 # 10GR / 100GR / 1000GR 
rho=1e-7 # Recombination rate (not scaled with GR!)
...
while ((fixed==0))
do 
slim -d Ne=$Ne -d rho=$rho -d rep=$rep -d s=$s  -d h=$h -d mu=$mu -d f=$f -d GR=$GR sln_sweep.slim > $tmpSLiM
